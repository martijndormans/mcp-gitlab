...
async function getMergeRequestRawDiff(
  ...
  const url = new URL(
    `${GITLAB_API_URL}/projects/${encodeURIComponent(
      projectId
    )}/merge_requests/${encodeURIComponent(mergeRequestId)}/raw_diffs`
  );
...
async function getJobLogs(projectId: string, jobId: string): Promise<string> {
  const url = new URL(
    `${GITLAB_API_URL}/projects/${encodeURIComponent(
      projectId
    )}/jobs/${encodeURIComponent(jobId)}/trace`
  );
  ...
  try {
    ...
    switch (request.params.name) {
      ...
      case "get_merge_request_raw_diff": {
        const args = GetMergeRequestRawDiffSchema.parse(
        ...
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify(mergeRequestsRawDiff, null, 2),
            },
          ],
        };
      }
      ...
      case "get_job_logs": {
        ...
        const logs = await getJobLogs(args.project_id, args.job_id);
        return {
